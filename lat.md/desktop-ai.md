...
[local-process-worker.mjs](../apps/desktop/src/local-process-worker.mjs) is a separate managed-Node supervisor: it owns the sandboxed process group, deadline, and aggregate 10 MB output limit. IPC disconnect on app exit kills that process group. Workers are unpacked beside the native runtimes. Scripts and local MCP operations share a four-process limit; scripts run for at most ten minutes; no background daemon or hosted executor is created.

[local-process-jobs.mjs](../apps/desktop/src/local-process-jobs.mjs) owns process ids, stdin, cancellation, a 1 MB/512-event output tail, 20 KB response pages, and completed records beneath the capability's receipt directory. `interact_local_process` takes a process id, cursor, wait duration, and a discriminated `action` object to read, write/close stdin, or terminate. Its root object remains compatible with provider function schemas. Stdin uses pipes; terminal emulation is not provided. App restart marks unfinished records interrupted without replaying their scripts. Completed results remain readable until capability revocation. Revoking or replacing the folder waits for admitted tool calls, stops its jobs, and removes their records. Output schemas and model instructions live in the shared local tool catalog; hosted workers never execute these scripts.
...
Local MCP discovery and calls launch managed stdio servers from the shared folder under the same native isolation, without an always-running service.

`list_local_mcp_tools` reads a bounded `.mcp.json` inventory. Its `mcpServers` object maps names to `{ "command": "node" | "python3", "args": ["relative/server/script", ...] }`; at most twenty servers may be configured. Only the managed interpreters are supported, and the script must resolve inside the authorized root. Omitted configuration gives an empty inventory; malformed configuration fails explicitly. Shell commands, environment overrides, package installation, and access to other user folders are not supported by this local contract. Existing remote HTTP MCP integrations retain their hosted authorization and network behavior.

[local-mcp-operations.mjs](../packages/ai/src/local-mcp-operations.mjs) owns configuration parsing, folder checks, paged discovery, and the hash required by `call_local_mcp_tool`. Cursors bind the root/configuration hash and server name; configuration changes require rediscovery. [local-mcp-client.mjs](../apps/desktop/src/local-mcp-client.mjs) uses the installed AI SDK client with retries disabled. Its [stdio transport](../apps/desktop/src/local-mcp-transport.mjs) runs through the native supervisor, validates JSON-RPC using the SDK, bounds each protocol message to 1 MB, and closes after each operation or failure. Each operation has a 30-second process deadline and shares the four-process native admission limit with script jobs. File-backed state persists; server memory is scoped to one operation. Capability revocation drains already-admitted operations before removing receipts.

The call tool explicitly uses non-strict function arguments because discovered servers own their arbitrary argument keys; its desktop boundary still validates the JSON contract. Model-facing responses are capped at 120 KB. Larger results must be written to shared-folder files by the server and read with existing file tools. The shared [MCP output converter](../packages/ai/src/mcp-tool-output.mjs) preserves text, images, structured content, and error status for local and remote tools. Durable capability receipts prevent replaying a tool call after success or interruption; restarting a server for a new operation never retries an earlier call.
