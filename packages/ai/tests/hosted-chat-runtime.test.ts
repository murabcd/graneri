...
describe("hosted chat route contract", () => {
	...
	it("validates the app-server-style steer route contract", () => {
		expect(
			...
			ok: true,
			intent: { type: "direct", continueRunId: null },
		});
		expect(
			parseHostedChatTurnIntent({
				continueRunId: null,
		...
		expect(
			parseHostedChatTurnIntent({
				...
				isSteerRoute: false,
				replayQueuedMessageId: "queued-replay-1",
				replayQueuedMessageStatus: "claimed",
				steerQueuedMessageId: null,
			}),
		).toMatchObject({
			errorCode: "replay_queued_message_status_invalid",
	...
	])("maps %s queued lifecycle errors to route conflicts", (code) => {
		const error = Object.assign(new Error("Queued lifecycle conflict."), {
			data: { code, message: "Queued lifecycle conflict." },
		});
	...
	])("maps %s queue invariant errors to server failures", (code) => {
		const error = Object.assign(new Error("Queue invariant failed."), {
			data: { code, message: "Queue invariant failed." },
		});

		expect(getHostedChatConvexRouteError(error)).toEqual({
			error: "Queue invariant failed.",
			errorCode: code,
			statusCode: 500,
		});
	});
	...
	it("maps missing Convex chat API functions to deployment skew route errors", () => {
		for (const missingFunction of [
			...
			"assistantQueuedMessages:claimForSteer",
